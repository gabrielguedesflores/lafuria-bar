...
const handlers = async () => {
        ...
        const orderDetailsHtml = `
            <form id="editOrderForm" class="row g-3">
                ...
                <div class="col-6" id="div-inputTotal">
                    ...
                    <input type="text" class="form-control" id="editOrderForm-inputTotal" value="${order.total}">
                </div>
    ...
    $(document).on('submit', '#editOrderForm', async function (e) {
        ...
        const orderId = $('.order-detail-button').data('order-id');

        // Aqui você precisa obter as novas informações do formulário e atualizar o pedido no servidor
        const newCustomer = $('#editOrderCustomer').val();
        const newNotes = $('#editOrderNotes').val();
        // Obtenha os novos valores para outras propriedades do pedido aqui

        // Atualize o pedido no servidor
        const updatedOrder = {
            _id: orderId,
            customer: newCustomer,
            notes: newNotes,
            // Inclua as novas propriedades do pedido aqui
        };

        try {
            const { data } = await axios.put(baseUrl + '/orders/' + orderId, updatedOrder);
            // Atualize o pedido na variável "orders"
